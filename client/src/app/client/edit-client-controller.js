	...
	'$scope', 'partyService', 'partyGroupService', 'rateMatrixService', '$state', 'client', 'clientGroup', 'rateMatrix', '$modal', '$filter', '$mdDialog', '$mdToast', '$stateParams', 'validationService', 'dialogService', function (
		...
		$scope.validateRateForm = function () {
			...
			for (var i = 0; i < keys.length; i++) {
				...
				if (object.taskType.formula.code === 'DISCOUNT_CALCULATOR' && !_.isNil(object.inputParameters.discountPercentage) && (object.inputParameters.discountPercentage < 0 || object.inputParameters.discountPercentage > 100)) {
					infoDialog('client.rates.invalidDiscount');
					return false;
				}
			}
			return true;
