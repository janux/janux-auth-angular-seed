...
module.exports = ['$scope','dialogService','userActionService','userService','$mdToast','$filter','$state','$timeout',
	function ($scope,dialogService, userActionService, userService, $mdToast, $filter , $state , $timeout) {
						...
						if (!_.isNil(emails) && emails.length > 0) {
							...
							_.every(emails, function (email) {
								...
								if(email.address !== '') {
									...
									userActionService.recoverPassword(userFound.userId, userFound.contact.id, recoverEmail).then(function(resp) {
										$mdToast.show(
											$mdToast.simple()
