...
describe("AuthService", function() {
	...
	before(function(done) {
		AuthService.loadAuthorizationContexts().then(function(contexts) {
	...
	describe("WIDGET permission context", function(done) {
		it("should have proper values", function() {
			...
			expect(authContext.description).to.equal(
				"Defines permissions available on a Widget that we want to track in our system"
			);
	...
	describe("ROLE permission context", function(done) {
		it("should have proper values", function() {
			...
			expect(authContext.description).to.equal(
				"Defines permissions available on a Role in our system or business domain"
			);
			var perms = authContext.getPermissionBitsAsList();
			expect(perms).to.have.length(5);
