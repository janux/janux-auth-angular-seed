...
var AuthGenerator = (function() {
	...
	AuthGenerator.generateAuthDataInTheDatabase = function(dbEngine, path) {
		...
		if (dbEngine === DataSourceHandler.MONGOOSE) {
			authContextDao = DaoFactory.subscribeDao(
				new DaoSettings(
					dbEngine,
					path,
					AUTHCONTEXT_DEFAULT_COLLECTION_NAME,
					EntityPropertiesImpl.createDefaultProperties(),
					AuthContextMongooseDbSchema
				),
				AuthContextDaoMongooseImpl
			);
			roleDao = DaoFactory.subscribeDao(
				new DaoSettings(
					dbEngine,
					path,
			...
			groupDao = DaoFactory.subscribeDao(
				new DaoSettings(
					dbEngine,
					path,
			...
			groupContentDao = DaoFactory.subscribeDao(
				new DaoSettings(
					dbEngine,
					path,
			...
			groupAttributeValueDao = DaoFactory.subscribeDao(
				new DaoSettings(
					dbEngine,
					path,
		...
		var inserted = new Promise(function(resolve) {
			...
			setTimeout(function() {
				...
				for (var iRole in authDataToInsert.roles) {
					roleToInsert.push(roleService.insert(authDataToInsert.roles[iRole].toJSON()));
				}
	...
	AuthGenerator.generateAuthData = function() {
		...
		_.each(standardAuthContextSetup, function(represents, name) {
			authData.authorizationContexts[name] = AuthorizationContext.createInstance(
				name,
				"Defines permissions available on a " + represents,
		...
		//
		authData.authorizationContexts["USER"].addPermissionBit(
			"GRANT",
			util.format("Grants permission to GRANT authorizations to Users in our system")
		);

		authData.roles.WIDGET_DESIGNER = Role.createInstance(
			"WIDGET_DESIGNER",
			"A person who creates and edits widget information"
		...
		authData.roles.SECURITY_MANAGER = Role.createInstance(
			"SECURITY_MANAGER",
			"A Person who can manage users and the authorization schema"
		)
